...
function Card(props) {
    ...
    const { card, setFav, selectCurrentCardPic, currentCardPics, setContact, sentMails, mailHasBeenSent } = props;
    ...
    return (<div className={'card' + topBorderClass}>
        ...
        <div className={cardLeft}>
            ...
            <div className="pic-footer">
                ...
                {card.posting_prices[0].expenses && <div className="expensas">{"+ " + Helper.ConvertirMonto(card.posting_prices[0].expenses.amount, card.posting_prices[0].expenses.currency) + " Expensas"}</div>}
            </div>
        </div>
